  ...
  self.client.bind(user, req.body.pwd, function(err) {
    if (err) {
      if (self._options.debug) {
        console.log('(EE) [ldapjs] LDAP bind error:', err.stack);
      }
    ...
    if (self._options.authOnly) {
      if (self._options.debug) {
        console.log('(II) [ldapjs] auth success:', req.body.user);
      }
      self.success({uid: req.body.user});
    } else {
      var search = {filter: self._options.search.filter.replace(/\$uid\$/, req.body.user)};
      ...
      self.client.search(dn, search, function(err, res) {
        if (err) {
          if (self._options.debug) {
            console.log('(EE) [ldapjs] LDAP error:', err.stack);
          }
          return self.fail(403);
        }
        ...
        res.on('searchEntry', function(entry) {
          ...
          self._verify(profile, function(err, user) {
            ...
            if (!user) {
              if (self._options.debug) {
                console.log('(EE) [ldapjs] LDAP user error:', self._challenge());
              }
              return self.fail(self._challenge());
            }
            if (self._options.debug) {
              console.log('(II) [ldapjs] auth success:', user);
            }
            self.success(user);
          });
        });
        ...
        res.on('error', function(err) {
          if (self._options.debug) {
            console.log('(EE) [ldapjs] Network error:', err.stack);
          }
          self.error(err);
        });
        ...
        res.on('end', function(result) {
          if (result.status !== 0) {
            if (self._options.debug) {
              console.log('(EE) [ldapjs] Result not OK:', result);
            }
