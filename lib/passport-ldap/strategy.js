...
Strategy.prototype.authenticate = function(req, options) {
  ...
  self.client.bind(user, req.body.pwd, function(err) {
    if (err) {
      debug('(EE) [ldapjs] LDAP bind error: %j', err.stack);
      return self.fail(403);
    }

    if (self._options.authOnly) {
      debug('(II) [ldapjs] auth success: %j', req.body.user);
      ...
      self.client.search(dn, search, function(err, res) {
        if (err) {
          debug('(EE) [ldapjs] LDAP error: %j', err.stack);
          return self.fail(403);
        }
        var items = [];
        ...
        res.on('error', function(err) {
          debug('(EE) [ldapjs] Network error: %j', err.stack);
          self.error(err);
        });

        res.on('end', function(result) {
          if (result.status !== 0) {
            debug('(EE) [ldapjs] Result not OK: %j', result);
            self.fail(result.status);
          }
          switch(items.length) {
            case 0:
              debug('(EE) [ldapjs] No results returned: %j', err.stack);
              return self.fail('No results returned');
            case 1:
              return self._verify(items[0], function(err, user) {
                ...
                if (!user) {
                  debug('(EE) [ldapjs] LDAP user error: %j', self._challenge());
                  return self.fail(self._challenge());
                }
                debug('(II) [ldapjs] auth success:', user);
              });
            case 2:
              debug('(EE) [ldapjs] Unexpected number of matches (%s) for "%s" username:', items.length, user);
